...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	private void saveTxt() {
		...
		File outputFile = fileChooser.getOutputFile(this, "Select File Name", "New_Text.txt");
		if (outputFile == null) {
			return;
		}
		...
		try {
			writer.println("TextCollage 1.0");
			Color bgColor = canvas.getBackground();
			writer.println("background " + bgColor.getRed() + " " +
					bgColor.getGreen() + " " + bgColor.getBlue());
			for (DrawTextItem element : theString) {
				writer.println();
				writer.println("start-string");
				writer.println("  text " + element.getString());

				Color textColor = element.getTextColor();

				writer.println("  color " + textColor.getRed() + " " +
						textColor.getGreen() + " " + textColor.getBlue());
				writer.println("  coordinates " + element.getX() + " " + element.getY());
	...
	private void openTxt() {
		...
		File toBeOpened = fileChooser.getInputFile(this, "Select File to be Opened");
		if (toBeOpened == null) {
			return;
		}
		...
		try {
			Reader reader = new BufferedReader(new FileReader(toBeOpened));
		...
		try {
			...
			while (scanner.hasNext()) {
				...
				if (elementName.equalsIgnoreCase("background")) {
					int red = scanner.nextInt();
					int green = scanner.nextInt();
					int blue = scanner.nextInt();
					newBgColor = new Color(red, green, blue);
				} else if (elementName.equalsIgnoreCase("start-string")) {
					scanner.next();
					String text = scanner.nextLine();
					DrawTextItem entry = new DrawTextItem(text);

					elementName = scanner.next();
					while (! elementName.equalsIgnoreCase("end-string")) {
						if (elementName.equalsIgnoreCase("color")) {
